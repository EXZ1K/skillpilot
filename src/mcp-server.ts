...
server.registerTool(
  ...
  async ({ categories }) => {
    ...
    const lines: string[] = [
      "# SkillForge — Available Agent Categories",
      "",
    ...
    lines.push("---");
    lines.push("To install agents, call `skillforge_install` with the chosen agent IDs.");
...
server.registerTool(
  "skillforge_install",
  {
    title: "SkillForge Install",
    description:
      "Generates .mcp.json, .env.example, and ROADMAP.md for the chosen agents. " +
      "Call skillforge_catalog first to see available agents. " +
  ...
  async ({ agentIds, targetDir, projectDescription }) => {
    ...
    try {
      ...
      await mkdir(join(targetDir, ".skillforge"), { recursive: true });
      await writeFile(
        join(targetDir, ".skillforge", "config.json"),
        JSON.stringify({
          version: "0.1.0",
          projectDescription,
          agents: agentIds,
      ...
      results.push("✓ .skillforge/config.json — metadata");
    ...
    // Итог
    const summary = [
      `# SkillForge — Installed ${chosen.length} agents`,
      "",
      ...results,
      "",
      "## Agents installed:",
...
function buildEnvExample(agents: SkillAlternative[]): string {
  const lines: string[] = ["# SkillForge — API Keys", ""];
  const seen = new Set<string>();
...
async function searchGitHub(
  ...
  const headers: Record<string, string> = {
    ...
    "User-Agent": "skillforge-mcp/0.1.0",
  };
...
server.registerTool(
  "skillforge_plan",
  {
    title: "SkillForge — Full Project Plan (one request → complete roadmap)",
    description:
  ...
  async ({ description, categories, techStack, ide }) => {
    ...
    lines.push("<!-- SkillForge Plan: use this as your implementation blueprint. -->");
    lines.push("<!-- Install the listed MCP agents, apply skills, and follow the roadmap. -->");
    lines.push("");
    lines.push(`# SkillForge Project Plan`);
    lines.push("");
    ...
    if (discoveredProjects.length > 0) {
      ...
      lines.push(`- \`skillforge_projects("${coreKeyword} ${stack || "template"}")\``);
    ...
    lines.push("- Install MCP agents: `skillforge_install` with chosen agent IDs");
    ...
    lines.push("");
    lines.push("To install all agents: `skillforge_install` with IDs:");
    lines.push(`\`[${catalogAgents.map((a) => `"${a.agent.id}"`).join(", ")}]\``);
    lines.push("");
    lines.push("---");
    lines.push("*Plan generated by SkillForge. Review the agents and roadmap above, then proceed with implementation.*");
